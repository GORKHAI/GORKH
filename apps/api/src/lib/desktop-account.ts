...
export async function buildDesktopAccountSnapshot(input: BuildDesktopAccountSnapshotInput) {
  ...
  return {
    ...
    billing: {
      ...
      hostedFreeAiPlan: input.user.subscriptionStatus === 'active' ? 'plus' : 'free',
      freeHostedTaskLimit: input.user.subscriptionStatus === 'active' ? null : 5,
      visionIncluded: input.user.subscriptionStatus === 'active',
    },
    currentDevice,
